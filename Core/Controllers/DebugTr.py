...
pak = requests.post('http://localhost/BBINCO/TV_DRIFT/Core/Controllers/PY.php', data=payload)
...
def start(day, pos):
    ...
    for ids in range(int(pakk['Pack']), paquetes):
        ...
        with open('/var/www/html/BBINCO/TV_DRIFT/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json', 'w', encoding='ascii') as file:
            json.dump(data, file, indent=4)

        with open('/var/www/html/BBINCO/TV_DRIFT/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json', 'r') as file:
            filedata = file.read()

        filedata = filedata.replace('[', '').replace(']', '')
        with open('/var/www/html/BBINCO/TV_DRIFT/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json', 'w') as file:
            file.write(filedata)
            print('/var/www/html/BBINCO/TV_DRIFT/Core/Controllers/Epg/'+IDF['IDF']+'/epg_'+day.strftime("%Y%m%d") + '_' + str(ids) + '.json ', 'CREADO')
...
for pos in range(len(listDays)):
    ...
    req = requests.post('http://localhost/BBINCO/TV_DRIFT/Core/Controllers/Packages.php', data=info)

# os.system ("python3 /var/www/html/BBINCO/TV_DRIFT/Core/Controllers/main.py")
