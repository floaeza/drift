...
if(isset($_SESSION['UserId'])){
    ...
    $ProfileName = $_SESSION['ProfileName'];
        ...
        $ImagesList="";
        $id = 0;
        $filehandle = opendir('/var/www/html/Multimedia/Homeowners/'); // Abrir archivos
...
//                $id++;
        ...
        foreach ($list as $fileName) {
            ...
            if($ext[1] == 'mp4' || $ext[1] == 'mov' || $ext[1] == 'MP4' ||  $ext[1] == 'MOV' ) {
            //$ImagesList.= '<img id="'.$id.'" style="min-height:90px; background-size: 100% 100%; background-image: url('."'".'http://10.0.3.10/MULTIMEDIA_VDM/Icons/video.png'."'".'); " class="ui image" src="/MULTIMEDIA_VDM/Module1/Monday/'.$fileName.'" tabindex="1" onclick="Selection();" title="'.$fileName.'">';
            $ImagesList.='<div class="ui rounded image" style="width:23%; height:150px; background-color: #B4B4B4;" id="'.$id.'div"><video id="'.$id.'" width="150px"  style="margin:2%;" onclick="Selection();" src="/Multimedia/Realestate/'.$fileName.'" muted controls></video><p style="font-size: 11px;">'.$fileName.'</p></div>';
            
            } else {
            $ImagesList.= '<div class="ui rounded image" style="width:23%; height:150px; "><img id="'.$id.'" style="max-height:90px;" class="ui image" src="/Multimedia/Realestate/'.$fileName.'" tabindex="1" onclick="Selection();" title="'.$fileName.'"><p style="font-size: 11px;">'.$fileName.'</p></div>';
            }
            $id++;   
        }        
        $Content = new Templates($Directory.'/Views/Templates/Module2_Lunes.tpl');
