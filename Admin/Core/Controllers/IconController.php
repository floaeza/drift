<?php
/* *********************************************************************************
 * OBJETIVO: Crear un enlace entre la vista Icon.tpl y sus modelos correspondientes
...
if(isset($_SESSION['UserId'])){
    ...
    $ProfileName = $_SESSION['ProfileName'];
        ...
        while ($file = readdir($filehandle)) {
            if ($file != "." && $file != "..") {
                $ImagesList.= '<img id="'.$id.'" style="max-height:90px;" class="ui rounded image" src="'.$IconsURL.$file.'" tabindex="1" onclick="Selection();" title="'.$file.'">';
                $id++;
            }else{ }
        ...
        $Content = new Templates($Directory.'/Views/Templates/Icon.tpl');
        
        /*Lista de Imagenes*/
        $Content->set('ListImages', $ImagesList);
        /* TEXTO DE LAS ETIQUETAS */
        $Content->set('LabelHeaderMultimedia', $Language['LabelHeaderMultimedia']);
        $Content->set('LabelSubHeaderMultimedia', $Language['LabelSubHeaderIcon']);
        //$Content->set('LabelTabMultimediaList', $Language['LabelTabIconList']);
